...
extension NibLoadable where Self : UIView {
    /**
     ...
     - bundle: The bundle containing the nib. Defaults to the main bundle.
     ...
     */
    public static func fromNib(name: String = String(describing: Self.self), owner: AnyObject? = nil, bundle: Bundle = Bundle.main) -> UIView? {
...
public protocol ContentViewLoadable : class {
    /**
     Called after the view has been loaded from the nib, had its outlets set, and added to the view hierarchy. Optional, the default implementation does nothing.
     */
...
extension ContentViewLoadable where Self : UIView {
    /**
     Loads the content view for this class, sets its outlets and actions, and adds it to the view hierarchy with the constraints of this object.
     
     `contentViewDidLoad()` is called when this is finished, and should be used to customise the view if required.
     */
    public func loadContentViewFromNib(in bundle: Bundle = Bundle.main) {
        if let view = Self.fromNib(owner: self, bundle: bundle) {
            view.frame = bounds
